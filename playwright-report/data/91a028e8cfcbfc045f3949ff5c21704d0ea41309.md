...
- Name: purchase-flow.spec.ts >> Purchase Flow >> checkout API is called with correct data on valid submit
- Location: tests/e2e/purchase-flow.spec.ts:92:7
...
Call log:
  - waiting for getByRole('button', { name: /proceed to pay/i })
...
- generic [ref=e1]:
  ...
  - dialog "Complete Purchase":
    - generic [ref=e882]:
      - generic [ref=e883]:
        - generic [ref=e884]:
          - heading "Complete Purchase" [level=2] [ref=e885]
          - paragraph [ref=e886]: Deep Dive Into Go · One-time ₹149
        - button "Close" [ref=e887]:
          - img [ref=e888]
      - generic [ref=e891]:
        - img "Deep Dive Into Go" [ref=e893]
        - generic [ref=e894]:
          - generic [ref=e895]: Deep Dive Into Go
          - generic [ref=e896]: 102 chapters · 315 programs · Lifetime access
        - generic [ref=e897]:
          - generic [ref=e898]: ₹149
          - generic [ref=e899]: ₹999
      - generic [ref=e900]:
        - generic [ref=e901]:
          - generic [ref=e902]:
            - generic [ref=e903]: First Name
            - textbox "First Name" [ref=e904]:
              - /placeholder: Arjun
              - text: Arjun
          - generic [ref=e905]:
            - generic [ref=e906]: Last Name
            - textbox "Last Name" [ref=e907]:
              - /placeholder: Mehta
              - text: Sharma
        - generic [ref=e908]:
          - generic [ref=e909]: Email Address
          - textbox "Email Address" [ref=e910]:
            - /placeholder: you@example.com
            - text: arjun@gmail.com
          - paragraph [ref=e911]:
            - img [ref=e912]
            - text: PDF delivered to this email instantly
        - generic [ref=e915]:
          - generic [ref=e916]: Mobile Number
          - generic [ref=e917]:
            - generic [ref=e918]:
              - combobox "Phone number country" [ref=e919] [cursor=pointer]:
                ...
                - option "Zimbabwe"
              - img [ref=e921]
            - textbox "Mobile Number" [active] [ref=e923]: +91 98765 43210
        - button "Buy Now ₹149" [ref=e924]:
          - img [ref=e925]
          - text: Buy Now ₹149
      - generic [ref=e927]:
        - generic [ref=e928]:
          ...
          - generic [ref=e938]:
            ...
            - text: UPI supported
        - generic [ref=e941]:
          - generic [ref=e942]: UPI
          - generic [ref=e943]: Cards
          - generic [ref=e944]: Net Banking
          - generic [ref=e945]: Wallets
        - paragraph [ref=e946]: Powered by Cashfree · 256-bit SSL encrypted
...
```ts
  26  |   test('clicking Buy Now opens the purchase modal', async ({ page }) => {
  27  |     const lp = new LandingPage(page);
  28  |     await lp.goto();
  29  | 
  30  |     await lp.openPurchaseModal();
  31  | 
  32  |     const modal = new PurchaseModalPO(page);
  33  |     await expect(modal.dialog).toBeVisible();
  34  |   });
  35  | 
  36  |   test('modal has correct ARIA dialog role', async ({ page }) => {
  37  |     const lp = new LandingPage(page);
  38  |     await lp.goto();
  39  |     await lp.openPurchaseModal();
  40  | 
  41  |     const dialog = page.getByRole('dialog');
  42  |     await expect(dialog).toHaveAttribute('aria-modal', 'true');
  43  |   });
  44  | 
  45  |   test('empty form submission shows validation errors', async ({ page }) => {
  46  |     const lp = new LandingPage(page);
  47  |     await lp.goto();
  48  |     await lp.openPurchaseModal();
  49  | 
  50  |     const modal = new PurchaseModalPO(page);
  51  |     await modal.submit();
  52  | 
  53  |     await expect(page.getByText(/at least 2 characters/i).first()).toBeVisible();
  54  |   });
  55  | 
  56  |   test('disposable email shows specific error', async ({ page }) => {
  57  |     const lp = new LandingPage(page);
  58  |     await lp.goto();
  59  |     await lp.openPurchaseModal();
  60  | 
  61  |     const modal = new PurchaseModalPO(page);
  62  |     await modal.firstNameInput.fill('Test');
  63  |     await modal.lastNameInput.fill('User');
  64  |     await modal.emailInput.fill('test@mailinator.com');
  65  |     await modal.submit();
  66  | 
  67  |     await expect(page.getByText(/temporary email/i)).toBeVisible();
  68  |   });
  69  | 
  70  |   test('close button (×) closes the modal', async ({ page }) => {
  71  |     const lp = new LandingPage(page);
  72  |     await lp.goto();
  73  |     await lp.openPurchaseModal();
  74  | 
  75  |     const modal = new PurchaseModalPO(page);
  76  |     await modal.close();
  77  | 
  78  |     await expect(modal.dialog).not.toBeVisible();
  79  |   });
  80  | 
  81  |   test('pressing Escape closes the modal', async ({ page }) => {
  82  |     const lp = new LandingPage(page);
  83  |     await lp.goto();
  84  |     await lp.openPurchaseModal();
  85  | 
  86  |     await page.keyboard.press('Escape');
  87  | 
  88  |     const modal = new PurchaseModalPO(page);
  89  |     await expect(modal.dialog).not.toBeVisible();
  90  |   });
  91  | 
  92  |   test('checkout API is called with correct data on valid submit', async ({ page }) => {
  93  |     let capturedBody: Record<string, unknown> = {};
  94  | 
  95  |     await page.route('/api/checkout', async (route) => {
  96  |       const postData = route.request().postDataJSON();
  97  |       capturedBody = postData;
  98  |       await route.fulfill({
  99  |         status: 200,
  100 |         contentType: 'application/json',
  101 |         body: JSON.stringify({ orderId: 'BLZ-TEST', paymentSessionId: 'session_x', cfOrderId: 'CF-1' }),
  102 |       });
  103 |     });
  104 | 
  105 |     // Mock cashfree-js load to prevent navigation
  106 |     await page.addInitScript(`
  107 |       window.__cashfreeCheckoutCalled = false;
  108 |       // Override dynamic import of cashfree-js
  109 |       const origImport = window.__import || function(){};
  110 |     `);
  111 | 
  112 |     const lp = new LandingPage(page);
  113 |     await lp.goto();
  114 |     await lp.openPurchaseModal();
  115 | 
  116 |     await page.getByLabel(/first name/i).fill('Arjun');
  117 |     await page.getByLabel(/last name/i).fill('Sharma');
  118 |     await page.getByLabel(/email address/i).fill('arjun@gmail.com');
  119 | 
  120 |     // Find phone input and fill
  121 |     const phoneInput = page.locator('input[name="phone"]').or(page.locator('.PhoneInputInput'));
  122 |     if (await phoneInput.count() > 0) {
  123 |       await phoneInput.first().fill('+919876543210');
  124 |     }
  125 | 
> 126 |     await page.getByRole('button', { name: /proceed to pay/i }).click();
      |                                                                 ^ Error: locator.click: Test timeout of 30000ms exceeded.
  127 | 
  128 |     await page.waitForTimeout(500);
  129 | 
  130 |     expect(capturedBody.firstName).toBe('Arjun');
  131 |     expect(capturedBody.lastName).toBe('Sharma');
  132 |     expect(capturedBody.email).toBe('arjun@gmail.com');
  133 |   });
  134 | 
  135 |   test('API error shows error message in modal', async ({ page }) => {
  136 |     await page.route('/api/checkout', (route) => {
  137 |       route.fulfill({
  138 |         status: 500,
  139 |         contentType: 'application/json',
  140 |         body: JSON.stringify({ error: 'Failed to create order' }),
  141 |       });
  142 |     });
  143 | 
  144 |     const lp = new LandingPage(page);
  145 |     await lp.goto();
  146 |     await lp.openPurchaseModal();
  147 | 
  148 |     await page.getByLabel(/first name/i).fill('Arjun');
  149 |     await page.getByLabel(/last name/i).fill('Sharma');
  150 |     await page.getByLabel(/email address/i).fill('arjun@gmail.com');
  151 |     await page.getByRole('button', { name: /proceed to pay/i }).click();
  152 | 
  153 |     await expect(page.getByText(/failed to create order/i)).toBeVisible();
  154 |   });
  155 | });
  156 | 
```