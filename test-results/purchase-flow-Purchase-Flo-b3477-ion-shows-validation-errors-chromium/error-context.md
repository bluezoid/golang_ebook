...
- Name: purchase-flow.spec.ts >> Purchase Flow >> empty form submission shows validation errors
- Location: tests/e2e/purchase-flow.spec.ts:45:7
      ...
      - generic [ref=e900]:
        - generic [ref=e901]:
          - generic [ref=e902]:
            ...
            - textbox "First Name" [active] [ref=e904]:
              - /placeholder: Arjun
          - generic [ref=e905]:
...
```ts
  ...
  2  | 
  3  | export class PurchaseModalPO {
  4  |   readonly page: Page;
  5  |   readonly dialog: Locator;
  ...
  11 |   readonly errorBox: Locator;
  12 | 
  13 |   constructor(page: Page) {
  14 |     this.page = page;
  15 |     this.dialog = page.getByRole('dialog');
  16 |     this.firstNameInput = page.getByLabel(/first name/i);
  17 |     this.lastNameInput = page.getByLabel(/last name/i);
  18 |     this.emailInput = page.getByLabel(/email address/i);
  19 |     this.submitButton = page.getByRole('button', { name: /proceed to pay/i });
  20 |     this.closeButton = page.getByRole('button', { name: /close/i });
  21 |     this.errorBox = page.locator('text=Failed to').first();
  22 |   }
  23 | 
  24 |   async fillForm(data: { firstName: string; lastName: string; email: string; phone?: string }) {
  25 |     await this.firstNameInput.fill(data.firstName);
  26 |     await this.lastNameInput.fill(data.lastName);
  27 |     await this.emailInput.fill(data.email);
  28 |   }
  29 | 
  30 |   async submit() {
> 31 |     await this.submitButton.click();
  ...
  37 | 
  38 |   async isVisible() {
  39 |     return this.dialog.isVisible();
  40 |   }
  41 | }
  42 | 
```