...
- Name: landing-page.spec.ts >> Landing Page >> loads and shows the hero heading
- Location: tests/e2e/landing-page.spec.ts:5:7
...
Locator: getByText('Deep Dive')
Expected: visible
Error: strict mode violation: getByText('Deep Dive') resolved to 5 elements:
    1) <h1 class="text-5xl lg:text-6xl xl:text-7xl font-bold tracking-tight text-white leading-[1.05]">…</h1> aka getByRole('heading', { name: 'Deep Dive Into Go' })
    2) <li class="flex items-center gap-2 text-sm text-zinc-400">…</li> aka getByText('sync package deep dive')
    3) <p class="text-sm text-zinc-500 leading-relaxed">Deep dive into the Go scheduler, goroutine intern…</p> aka getByText('Deep dive into the Go')
    4) <span class="text-xs font-medium text-zinc-500 uppercase tracking-wider">Deep Dive Into Go</span> aka getByText('Deep Dive Into Go', { exact: true })
    5) <p class="text-xs text-zinc-700">Deep Dive Into Go · Digital Product</p> aka getByText('Deep Dive Into Go · Digital')

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for getByText('Deep Dive')
